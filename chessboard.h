...
public:
    ...
    void set_restrict_level(int level); // level=0可以正常预选并落子
                                        ...
                                        // level=2不能进行任何操作
    // 刷新
    void paintEvent(QPaintEvent *); ///<根据相关数据绘制棋盘界面。\n 继承时不需要重载。\n UI设计需要修改此函数。
    // 鼠标移动
    void mouseMoveEvent(QMouseEvent *event); ///<鼠标移动时计算预选点。\n 不需要重载。
    // 落子
    virtual void mousePressEvent(QMouseEvent *event)=0; ///< 鼠标点击事件。\n 需要根据具体功能重载。@details 重载方法参考@ref Chessboard_Local_PVP::mousePressEvent


protected:
    Ui::ChessBoard *ui;
    // 超参数
    int STARTX = 100; ///<棋盘左上角的X值
    int STARTY = 70; ///<棋盘左上角的Y值
    int GRIDSIZE = 60; ///<棋盘上格子的大小
    int CHESSR = GRIDSIZE / 2 * 0.9; ///< 棋子半径
    int HINTR = GRIDSIZE / 8; ///< 落点提示半径
    int HINTR2 = GRIDSIZE / 9; ///< 指示最新落子的提示半径
    // 下棋控制
    /**
     * @brief 当前棋盘状态
