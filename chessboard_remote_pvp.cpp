...
Chessboard_Remote_PVP_Abstract::Chessboard_Remote_PVP_Abstract(
    Chess_color color, QWidget *parent, int new_game_mode)
    ...
    myLocalTime = new Time{
        ...
        [this]() { systemMessage("你的局时快要达到限制了,请快落子"); }};
    ...
    myStepTime = new Time{
        ...
        [this]() { systemMessage("你的步时快要达到限制了,请快落子"); }};
...
void Chessboard_Remote_PVP_Abstract::systemDo(const QJsonObject &order) {
    ...
    if (willDo == "chess") {
        ...
        if (int(record.size()) <
            peerRecordSize) {  // 如果我比对方的小,我不用动,等待对方撤回一个子
            return;
        } else if (
            int(record.size()) ==
            peerRecordSize) {  // 如果我和对方一样,为了防止悔棋的同意信号没有被发出,重新设置先后手
            // 重新设置先后手
...
void Chessboard_Remote_PVP_Abstract::win(const QString &info) {
    ...
    auto questionDialog =
        new QMessageBox(QMessageBox::Information, "游戏结束",
...
Chessboard_Remote_PVP_Abstract::Time &
Chessboard_Remote_PVP_Abstract::Time::operator++() {
    time++;
    ...
    if (timeLabel != nullptr)
        timeLabel->setText(prefix + QString::number(time / 60) + "分" +
                           QString::number(time % 60) + "秒");
    return *this;
}

void Chessboard_Remote_PVP_Abstract::Time::clear() {
    time = 0;
    if (timeLabel != nullptr) timeLabel->setText(prefix + "0分" + "0秒");
}
...
Chessboard_Remote_PVP_Server::~Chessboard_Remote_PVP_Server() { delete server; }

Chessboard_Remote_PVP_Server::Chessboard_Remote_PVP_Server(Chess_color color,
                                                           QWidget *parent,
                                                           int new_game_mode)
...
void Chessboard_Remote_PVP_Server::send(const QString &s) { server->send(s); }

void Chessboard_Remote_PVP_Server::exit() { server->stop(); }

void Chessboard_Remote_PVP_Server::setActiveExit() { server->setActiveExit(); }
...
Chessboard_Remote_PVP_Client::~Chessboard_Remote_PVP_Client() { delete client; }

Chessboard_Remote_PVP_Client::Chessboard_Remote_PVP_Client(
    Chess_color color, const QHostAddress &serverAddress, int serverPort,
...
void Chessboard_Remote_PVP_Client::send(const QString &s) { client->send(s); }

void Chessboard_Remote_PVP_Client::exit() { client->stop(); }

void Chessboard_Remote_PVP_Client::setActiveExit() { client->setActiveExit(); }
