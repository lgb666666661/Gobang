...
// 检查以(x, y)为中心的dir方向上是否是黑子活四
{
    ...
    for (int step = 1; step <= 4; step++) {
        ...
        //        qDebug() << "setting black:" << curx << cury << dir.x <<
        //        dir.y; qDebug() << "checking 5:" << x << y << dir.x << dir.y;
...
int ChessBoard::__getCnt(int x, int y, const Point& dir, int depth) {
    // 检查以(x, y)为中心的dir方向（双向）上连成的长度
    // 且必须包含record的最后depth个点 当depth = 0时，单纯检查以(x,
    // y)为中心的dir方向上连成的长度
...
void ChessBoard::paintEvent(QPaintEvent*) {
    ...
    QRectF boarder3(STARTX - tmp, STARTY - tmp, 14 * GRIDSIZE + 2 * tmp,
                    14 * GRIDSIZE + 2 * tmp);
    ...
    // "五子棋"logo
    QRectF boarder4(STARTX + 17 * GRIDSIZE, STARTY - GRIDSIZE, 7 * GRIDSIZE,
                    5 * GRIDSIZE);
    ...
    for (int i = 0; i < 15; i++) {
        for (int j = 0; j < 15; j++) {
            if (chessboard[i][j] == BLACK) {
                ...
                painter.drawEllipse(
                    {STARTX + i * GRIDSIZE, STARTY + j * GRIDSIZE}, CHESSR,
                    CHESSR);
            }
    ...
    if (nearx >= 0 && neary >= 0) {
        ...
        painter.drawEllipse(
            {STARTX + nearx * GRIDSIZE, STARTY + neary * GRIDSIZE}, HINTR,
            HINTR);
    }
    ...
    if (nowx >= 0 && nowy >= 0) {
        ...
        painter.setPen(pen);
        painter.setBrush(Qt::red);
        painter.drawEllipse({STARTX + nowx * GRIDSIZE,
                             STARTY + nowy * GRIDSIZE - int(GRIDSIZE * 0.3)},
                            HINTR2, HINTR2);
    }
...
void ChessBoard::mouseMoveEvent(QMouseEvent* event) {
    ...
    if (curx >= STARTX - THRES && curx <= STARTX + 14 * GRIDSIZE + THRES &&
        ...
        if ((curx - STARTX) % GRIDSIZE > THRES) nearx += 1;
        neary = (cury - STARTY) / GRIDSIZE;
        if ((cury - STARTY) % GRIDSIZE > THRES) neary += 1;

        if (chessboard[nearx][neary] != EMPTY) nearx = -1, neary = -1;
    }
...
void ChessBoard::set_restrict_level(int level) { restrict_level = level; }
